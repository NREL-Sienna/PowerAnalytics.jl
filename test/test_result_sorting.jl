...
@testset "test filter results" begin
    ...
    @test length(gen.data) == 4  # TODO will be 7 when storage and hydro are reenabled
    ...
    @test length(sub_gen.data) == 5  # TODO will be 8 when storage and hydro are reenabled
end
...
@testset "test data aggregation" begin
    ...
    @test length(fuel) == 7  # TODO will be 8 when storage and hydro are reenabled

    fuel_agg = PA.combine_categories(fuel)
    @test size(fuel_agg) == (48, 7)  # TODO will be (48, 8) when storage and hydro are reenabled
end
